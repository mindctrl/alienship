...
<?php
  ...
  <section id="respond">
    ...
    $comment_form_args = array(
      ...
      'comment_field'        => '<label for="comment" class="comment-label">' . _x( 'Comment', 'noun', 'alienship' ) . '</label><div class="input-prepend"><span class="add-on"><i class="glyphicon glyphicon-comment"></i></span><textarea class="input-large" id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></div>',
      ...
      'fields'               => apply_filters( 'comment_form_default_fields', array(
                                              'author' => '' . '<label for="author" class="comment-label">' . __( 'Name', 'alienship' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<div class="input-prepend"><span class="add-on"><i class="glyphicon glyphicon-user"></i> </span><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
                                              'email'  => '<label for="email" class="comment-label">' . __( 'Email', 'alienship' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<div class="input-prepend"><span class="add-on"><i class="glyphicon glyphicon-envelope"></i> </span><input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div>',
                                              'url'    => '<label for="url" class="comment-label">' . __( 'Website', 'alienship' ) . '</label>' . '<div class="input-prepend"><span class="add-on"><i class="glyphicon glyphicon-home"></i> </span><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></div>' )
                                              )
                );
